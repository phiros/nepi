    ...
    for a in "${apps[@]}"; do
        for t in "${threads[@]}"; do
            for i in $(seq $run0 $runn); do
                ...
                echo "NEPI_LOGLEVEL=info python benchmark/scalability/planetlab.py -n $n -a $a -t $t -r $i > logs/nodes$n.apps$a.threads$t.runs$i.out 2>&1"
                NEPI_LOGLEVEL=debug python benchmark/scalability/planetlab.py -n $n -a $a -t $t -r $i > logs/nodes$n.apps$a.threads$t.runs$i.out 2>&1
                if [ $? != 0 ]; then
